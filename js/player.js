!function($) {
    ...
    function play(id) {
        ...
        songInfo.children("h3").text(pangu.spacing(currentPlaying.name));
        songInfo.children("span").text(pangu.spacing(currentPlaying.artists));
        ...
        if (mainLrc != lastMainLrc) {
            ...
            setTimeout(function() {
                if (mainLrc == "") {
                    ...
                    player.lyric.find("h3").text(pangu.spacing(mainLrc));
                }
        ...
        if (subLrc != lastSubLrc) {
            ...
            setTimeout(function() {
                if (subLrc == "") {
                    ...
                    player.lyric.find("span").text(pangu.spacing(subLrc));
                }
    ...
    $(window).on("scroll",function() {
    	if ($("footer").offset().top-$(window).height()+30<$(window).scrollTop()) {
        ...
        if (player.find(".song-list").hasClass("show")) {
            ...
            if (songs.length != player.find(".song-list").children().length) {
                ...
                for (let i = 0;i<songs.length;i++) {
                    let song = songs[i];
                    player.find(".song-list").append("<div class='song' data-index='" + i + "'><img src='" + song.thumbnail.replace("http:","https:") + "?param=48y48'/><h5>" + pangu.spacing(song.name) + "</h5><span>" + pangu.spacing(song.artists) + "</span></div>");
                }
